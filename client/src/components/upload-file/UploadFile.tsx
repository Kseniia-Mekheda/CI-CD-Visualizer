    ...
    <div className="w-full max-w-xl mx-auto flex flex-col items-center">
      <div
        ...
        className={`group relative flex w-full cursor-pointer flex-col items-center justify-center rounded-3xl border-2 border-dashed bg-white p-12 transition-all ${
          isDragging
            ? 'border-accent bg-accent-light/50 scale-105'
            : 'border-light-border hover:border-accent hover:bg-accent-light/30'
        }`}
      >
        {isLoading ? (
          ...
          <>
            <div
              className={`mb-4 rounded-full p-4 transition-transform ${isDragging ? 'bg-accent text-white' : 'bg-accent-light text-accent group-hover:scale-110'}`}
            >
              <Upload size={32} />
            </div>
